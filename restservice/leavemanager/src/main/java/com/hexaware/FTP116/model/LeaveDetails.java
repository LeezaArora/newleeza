...
public class LeaveDetails {
  ...
  * default Constructor.
   ...
   * @param argbalanceEL initialize Status of applied Leave.
    *    @param argbalanceEL Pl initialize Status of applied Leave.
     *   @param argbalanceELl initialize Status of applied Leave.
  ...
  /* public LeaveDetails(final int argLeaveEmpId, final int argLeaveId, final LeaveType argLeaveType,
                      final Date argLeaveStartDate, final Date argLeaveEndDate, final Date argLeaveAppliedOn,
                      final String argLeaveReason, final int argLeaveNumOfDays, final LeaveStatus argLeaveStatus,
                     final int argEl ,final int argPl,final int argMl,final String argLeaveMgrComments)*/
 ...
 */
  public static String approveDeny(final int argLeaveId,
    ...
    if (ld != null) {
      ...
      LeaveType type =ld.getLeaveType();
      int leavAvail =0;
      if(LeaveType.EL== type){
        leavAvail = ed.getBalanceEl();
      }
     else if(LeaveType.PL== type){
        leavAvail = ed.getBalancePl();
      }
      if(LeaveType.ML== type){
        leavAvail = ed.getBalanceMl();
      }
    ...
    //  int leavAvail = ed.getEmpLeaveBalance();
      ...
      if (empManagerId != argEmpManagerId) {
        ...
        if (argLeaveStatus.equals("YES")) {
          ...
          dao().approveOrDeny(argLeaveId, leaveStatus, argLeaveMgrComments);
          if(LeaveType.EL == type) {
            edao().updateEl(empId, leavAvail);
          }
         else if(LeaveType.PL == type) {
            edao().updatePl(empId, leavAvail);
          }
            if(LeaveType.ML == type) {
            edao().updateMl(empId, leavAvail);
          }
  ...
  public static String applyLeave(final String argLeaveStartDate,
    ...
    //  int leavAvail = 0;
      if (LeaveType.EL == argLeaveType){
        leavAvail = e.getBalanceEl();
      }
    else if (LeaveType.ML == argLeaveType){
        leavAvail = e.getBalanceMl();
      }
    else if (LeaveType.PL == argLeaveType){
      leavAvail = e.getBalancePl();
      }
    //  int leavAvail = e.getEmpLeaveBalance();
      ...
      if (days <= 0) {
       ...
       if(LeaveType.EL == argLeaveType) {
        edao().updateEl(argEmpId, leavAvail);
        res = "leave AUTOAPPROVED";
             }
              else if(LeaveType.ML == argLeaveType) {
        edao().updateMl(argEmpId, leavAvail);
        res = "leave AUTOAPPROVED";
             }
       else if(LeaveType.PL == argLeaveType) {
        edao().updatePl(argEmpId, leavAvail);
        res = "leave AUTOAPPROVED";
             }
        else{
          res = "no chnge";
